...
class ActivationType(Enum):
    ...
    RELU_SQUARE = 4
    REQU = 5
    # tanh and sigmoid are dReal only
    TANH = 6
    SIGMOID = 7
    LIN_SQUARE_CUBIC = 8
...
def activation_der(select: ActivationType, p):
    ...
    if select == ActivationType.IDENTITY:
        ...
        return torch.ones(p.shape).double()
    elif select == ActivationType.SQUARE:
        return 2*p
    elif select == ActivationType.LIN_SQUARE:
...
def identity_der(x):
    return torch.ones(x.shape).double()
...
def lqc_der(x):
    ...
    h = int(x.shape[1] / 3)
    x1, x2, x3 = x[:, :h], x[:, h:2 * h], x[:, 2 * h:]
    return torch.cat((torch.ones(x1.shape).double(), 2 * x2, 3 * torch.pow(x3, 2)), dim=1)
...
def lqcq_der(x):
    ...
    h = int(x.shape[1] / 4)
    x1, x2, x3, x4 = x[:, :h], x[:, h:2*h], x[:, 2*h:3*h], x[:, 3*h:]
    return torch.cat((torch.ones(x1.shape).double(), 2*x2, 3*torch.pow(x3, 2), 4*torch.pow(x4,3)), dim=1)
...
def lqcqp_der(x):
    ...
    h = int(x.shape[1] /5)
    x1, x2, x3, x4, x5 = x[:, :h], x[:, h:2*h], x[:, 2*h:3*h], x[:, 3*h:4*h], x[:, 4*h:]
    return torch.cat((torch.ones(x1.shape).double(), 2*x2, 3*torch.pow(x3, 2), \
...
def l_o_der(x):
    ...
    x1, x2, x3, x4, x5, x6, x7, x8 = x[:, :h], x[:, h:2*h], x[:, 2*h:3*h], x[:, 3*h:4*h], \
                             x[:, 4*h:5*h], x[:, 5*h:6*h], x[:, 6*h:7*h], x[:, 7*h:]
    return torch.cat((torch.ones(x1.shape).double(), 2*x2, 3*torch.pow(x3, 2), 4*torch.pow(x4,3), \
